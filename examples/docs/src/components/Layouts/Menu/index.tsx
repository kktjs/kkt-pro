...
import { config, menusType, MenusConfigObject } from '@/config';
  ...
  data.forEach((item, idx) => {
    if (item.path) {
      if (item.target || /^https?:\/\//.test(path)) {
        result.push(
          <MenuA key={idx + (item.path || '')} href={item.path} target={item.target}>
            {item.title}
          </MenuA>,
        );
      } else {
...
const Menu = () => {
  ...
  const initMenu = () => {
    menus.forEach((item: menusType) => {
      const path = new RegExp(item.path);
      if (path.test(pathname)) {
        setResult(getRouters(item.child, pathname));
      }
    });
  };
