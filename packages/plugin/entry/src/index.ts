...
class InitEntry implements WebpackPluginInstance {
  ...
  queryClient: boolean = false;
  constructor({ cacheDirName, redux = false, queryClient = false }: InitEntryOptions = {}) {
    if (cacheDirName) {
  ...
  init() {
    ...
    const code = getInitCode({ redux: this.redux, queryClient: this.queryClient });
